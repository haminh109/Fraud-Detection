...
TOP_V_SIGNAL_COLS = ["V257", "V258", "V246", "V243", "V265", "V264", "V219"]
...
class TopVSignalTransformer(BaseEstimator, TransformerMixin):
    def __init__(self):
        self.v_stats_ = {}   
    ...
    def fit(self, X, y=None):
        for col in TOP_V_SIGNAL_COLS:
            if col in X.columns:
                ...
                self.v_stats_[col] = {'q99': q99, 'q01': q01}
        return self
    ...
    def transform(self, X):
        ...
        for col in TOP_V_SIGNAL_COLS:
            if col in df.columns:
                ...
                if col in self.v_stats_:
                    q99 = self.v_stats_[col]['q99']
                    q01 = self.v_stats_[col]['q01']
                    df[f"{col}_high_flag"] = (df[col] > q99).astype("int8")
                    df[f"{col}_low_flag"] = (df[col] < q01).astype("int8")
                else:
                    df[f"{col}_high_flag"] = 0
                    df[f"{col}_low_flag"] = 0
        return df
